...
delete_post.delete("/api/admin/news-and-blogs/delete/:id", async (req, res) => {
  const { id } = req.params;

  const query = `DELETE FROM news_and_blogs WHERE id = $1;`;
  ...
  if (error) {
    ...
    return res
      ...
      .json({
        success: false,
        error: "Failed to delete news or blog post",
      });
  }
